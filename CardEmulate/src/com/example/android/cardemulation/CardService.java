...
public class CardService extends HostApduService {
	...
	private static final String SE_AID = "69636663637365696400000000000001";
	...
	private static String CURRENT_AID = null;
	...
	public byte[] processCommandApdu(byte[] commandApdu, Bundle extras) {
		...
		if (ins == (byte) 0xA4) {
			CURRENT_AID = ByteArrayToHexString(commandApdu).substring(10);
			Log.i(TAG, "current AID= " + CURRENT_AID);
		}
		// If the APDU matches the SELECT AID command for this service,
		// send the loyalty card account number, followed by a SELECT_OK status
		// trailer (0x9000).
		if (CURRENT_AID.equals(SAMPLE_LOYALTY_CARD_AID)) {
			switch (ins) {
			case (byte) 0xA4:
				String account = AccountStorage.GetAccount(this);
				byte[] accountBytes = account.getBytes();
				Log.i(TAG, "Sending account number: " + account);
				response = accountBytes;
				status = ISO7816.SW_NO_ERROR;
				break;
			default:
				status = ISO7816.SW_INS_NOT_SUPPORTED;
				break;
			}
		} else if (CURRENT_AID.equals(SE_AID)) {
			switch (ins) {
			case (byte) 0xA4:
				byte[] seidBytes = SEApplet.getSEID();
				Log.i(TAG, "Sending account number: " + ByteArrayToHexString(seidBytes));
				if (seidBytes != null)
					response = seidBytes;
				status = ISO7816.SW_NO_ERROR;
				break;
			case (byte)0xE2:
				byte[] pseid = new byte[lc];
				System.arraycopy(commandApdu, 5, pseid, 0, lc);
				boolean ret= SEApplet.setSEID(pseid);
				if(ret)
					status = ISO7816.SW_NO_ERROR;
				else
					status = ISO7816.SW_CONDITIONS_NOT_SATISFIED;
				break;
			default:
				status = ISO7816.SW_INS_NOT_SUPPORTED;
				break;
		...
		if(response == null)
			return ShortoByteArray(status);
		else
			return ConcatArrays(response, ShortoByteArray(status));
	}
	...
	public static byte[] ShortoByteArray(short p)
			throws IllegalArgumentException {
